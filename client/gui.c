/* $Header: /home/jcb/MahJong/newmj/RCS/gui.c,v 12.21 2014/01/05 22:45:34 jcb Exp $
 * gui.c
...
static void vis_callback(GtkWidget *w UNUSED, GdkEvent *ev, gpointer data UNUSED)
{
  if ( ev->type == GDK_MAP ) {
	...
	if ( ww == NULL ) continue;
	/* not completely sure why this is needed, but it is */
#ifdef GTK2
	if ( ! GDK_IS_WINDOW((gtk_widget_get_window(ww) )) ) continue;
#endif
...
#endif
GtkWidget *highlittile = NULL ; /* the unique highlighted discard */
GtkWidget *highlitinhand = NULL; /* the highlighted tile to show that
				    that it's a player's turn */
...
main (int argc, char *argv[])
{
  ...
  /* we could potentially get SIGPIPE if we're passing server commands
     to stdout. We don't want to fail in that case. */
  ignore_sigpipe();
...
# endif
  ...
  */
  ...
  case CMsgWashOut:
	...
	uniconify_window(textwindow);
      }
    }
  }

  /* if the message was an error, or a closed set formation
     message about us, clear the closed_set_in_progress flag.
     The error case is dealt with above, since our error
  ...
  */
  {
    ...
    switch ( m->type ) {
    case CMsgPlayerFormsClosedSpecialSet:
      mid = ((CMsgPlayerFormsClosedSpecialSetMsg *)m)->id;
      break;
    case CMsgPlayerFormsClosedPair:
      mid = ((CMsgPlayerFormsClosedPairMsg *)m)->id;
      break;
    case CMsgPlayerFormsClosedPung:
      mid = ((CMsgPlayerFormsClosedPungMsg *)m)->id;
      break;
    case CMsgPlayerFormsClosedChow:
      mid = ((CMsgPlayerFormsClosedChowMsg *)m)->id;
      break;
    case CMsgPlayerShowsTiles:
      mid = ((CMsgPlayerShowsTilesMsg *)m)->id;
      break;
    default: ;
    }
    if ( mid == our_id ) {
  ...
  /* update the scoring history after a Game or Settlement message */
  
  if ( m->type == CMsgGame || m->type == CMsgSettlement ) {
#ifdef GTK2
    GtkTextBuffer *g = GTK_TEXT_BUFFER(gtk_text_view_get_buffer(GTK_TEXT_VIEW(scorehistorytext)));
    static GtkTextTag *tag = NULL;
    if ( tag == NULL ) {
      tag = gtk_text_buffer_create_tag(g,
	"underline","underline",PANGO_UNDERLINE_SINGLE,NULL);
    }
    GtkTextIter iter;
    gtk_text_buffer_get_end_iter(g,&iter);
    gtk_text_buffer_place_cursor(g,&iter);
#else
    int npos;
    GtkEditable *g = GTK_EDITABLE(scorehistorytext);
    gtk_text_set_point(GTK_TEXT(scorehistorytext),gtk_text_get_length(GTK_TEXT(scorehistorytext)));
#endif    
    char buf[80];
    int i;
    static seats lastround = noseat;
    ...
    if ( m->type == CMsgGame ) {
      ...
      gtk_text_buffer_set_text(g,"",0);
#else
      gtk_editable_delete_text(g,0,-1);
#endif
      /* build a line with the player names (in board order),
	 marking original east with @ */
      strcpy(buf," ");
      for (i=0; i < NUM_SEATS; i++) {
	PlayerP p = the_game->players[board_to_game(i)];
	sprintf(1+buf+16*i,"%13.13s%c%s",p->name,
	  (p->id == the_game->firsteast ? '@' : ' '),
	  (i == NUM_SEATS-1 ? "\n" : "  "));
      }
#ifdef GTK2
#define ADDBUF    gtk_text_buffer_insert_at_cursor(g,buf,strlen(buf))
#else
#define ADDBUF    gtk_editable_insert_text(g,buf,strlen(buf),&npos)
#endif
      ADDBUF;
      /* now the initial scores */
      strcpy(buf," ");
      for (i=0; i < NUM_SEATS; i++) {
	PlayerP p = the_game->players[board_to_game(i)];
	sprintf(1+buf+16*i,"%13d %s",p->cumulative_score,
	  (i == NUM_SEATS-1 ? "\n" : "  "));
      }
      ADDBUF;
      /* and state the round */
      ...
      /* for settlement, display the handscores and the change,
	 and then the scores. Underline the changes. */
      int chg[NUM_SEATS];
      CMsgSettlementMsg *sm = (CMsgSettlementMsg *)m;
      chg[east] = sm->east; chg[south] = sm->south;
      chg[west] = sm->west; chg[north] = sm->north;
      if ( lastround != the_game->round ) {
	sprintf(buf,"%s round\n",windnames[lastround = the_game->round]);
	ADDBUF;
      }
      for (i=0; i < NUM_SEATS; i++) {
	...
	sprintf(buf,"%s%s%c",
	  (i == 0 ? " " : ""),
	  shortwindnames[p->wind],
	  (the_game->player == board_to_game(i) ? '*' : ' '));
	ADDBUF;
	int len;
	sprintf(buf,"%d",p->hand_score);
	len = strlen(buf);
	sprintf(buf,"%*s(%d)",4-len,"",p->hand_score);
	ADDBUF;
	sprintf(buf,"%+5d",chg[board_to_game(i)]);
#ifdef GTK2
	GtkTextIter iter;
	gtk_text_buffer_get_end_iter(g,&iter);
	gtk_text_buffer_insert_with_tags(g,&iter,
	  buf,strlen(buf),tag,NULL);
	gtk_text_buffer_get_end_iter(g,&iter);
	gtk_text_buffer_place_cursor(g,&iter);
#else
	ADDBUF;
#endif
	strcpy(buf,(i < NUM_SEATS-1 ? "   " : " \n"));
	ADDBUF;
      }
#ifndef GTK2
      strcpy(buf,"         -----          -----          -----          ----- \n");
      ADDBUF;
#endif
      /* and now the cumulative scores again */
      strcpy(buf," ");
      for (i=0; i < NUM_SEATS; i++) {
	PlayerP p = the_game->players[board_to_game(i)];
	sprintf(1+buf+16*i,"%13d %s",p->cumulative_score,
    ...
    /* force window to scroll to end */
#ifdef GTK2
    static GtkTextMark *mark = NULL;
    if ( mark == NULL ) {
      GtkTextIter eiter;
      gtk_text_buffer_get_end_iter(g,&eiter);
      mark = gtk_text_buffer_create_mark(g,"end",&eiter,FALSE);
    }
    gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(scorehistorytext),
...
void setup_dialogs(void) {
  ...
  */
  ...
  if ( !auto_declaring_hand
    ...
    /* to reduce confusing, make it insensitive if we've made a request
       and are waiting for the reply */
    gtk_widget_set_sensitive(scoring_dialog,!closed_set_in_progress);
  } else {
...
static int playerdisp_update_concealed(PlayerDisp *pd,Tile t) {
  ...
  /* if it's the player's turn, other than ours, highlight their
     rightmost tile to indicate this */
  if ( the_game != NULL ) {
    if ( the_game->player != our_seat
      && p == the_game->players[the_game->player] ) {
      if ( the_game->state == DeclaringSpecials
	|| the_game->state == Discarding ) {
	GtkWidget *t = pd->conc[p->num_concealed-1];
	if ( t != highlitinhand && highlitinhand != NULL ) {
#ifdef GTK2
	  gtk_widget_modify_bg(highlitinhand,GTK_STATE_NORMAL,NULL);
#else
	  gtk_widget_restore_default_style(highlitinhand);
#endif
	  highlitinhand = NULL;
	}
#ifdef GTK2
	gtk_widget_modify_bg(highlitinhand = t,GTK_STATE_NORMAL,&highlightcolor);
#else
	gtk_widget_set_style(highlitinhand = t,highlightstyle);
#endif
      } else {
	if ( highlitinhand != NULL ) {
#ifdef GTK2
	  gtk_widget_modify_bg(highlitinhand,GTK_STATE_NORMAL,NULL);
#else
	   gtk_widget_restore_default_style(highlitinhand);
    ...
    } else if ( the_game->player == our_seat ) {
...
static void playerdisp_update(PlayerDisp *pd, CMsgUnion *m) {
  ...
  if ( m->type == CMsgPlayerDeclaresSpecial ) {
	...
	if ( is_special(our_player->concealed[our_player->num_concealed-1]) ) {
      ...
      /* update the concealed tiles of the *next* player, because
	 it's their turn - this gets highlighting correct */
      playerdisp_update_concealed(&pdisps[game_to_board(the_game->player)],HiddenTile);
    }
...
#     endif
  ...
  */
  ...
  /* and the scoring history window */
  scorehistory_init();
  ...
  /* and if there's a current connection, set callbacks
     (also on stdin if passing stdin) */
  control_server_processing(1);
...
void control_server_processing(int on) {
  if ( on && the_game && the_game->fd != (int)INVALID_SOCKET ) {
    ...
    if ( stdin_callback_tag ) {
      gdk_input_remove(stdin_callback_tag);
      stdin_callback_tag = 0;
    }
  }
}
